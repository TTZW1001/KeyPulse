...
public sealed class RawMouseParser
{
        ...
        ICollection<InputEvent> output)
    {
        ...
        {
            output.Add(Button(MouseButton.Left, timestamp));
        }

        if ((buttonFlags & RightDown) != 0)
        {
            output.Add(Button(MouseButton.Right, timestamp));
        }

        if ((buttonFlags & MiddleDown) != 0)
        {
            output.Add(Button(MouseButton.Middle, timestamp));
        }

        if ((buttonFlags & X1Down) != 0)
        {
            output.Add(Button(MouseButton.XButton1, timestamp));
        }

        if ((buttonFlags & X2Down) != 0)
        {
            output.Add(Button(MouseButton.XButton2, timestamp));
        }
    ...
    private static MouseButtonEvent Button(MouseButton button, DateTimeOffset timestamp) =>
        new()
        {
            Timestamp = timestamp,
            Button = button
